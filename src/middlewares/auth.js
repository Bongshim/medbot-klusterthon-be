...
const getRolesAndPermissions = async (user) => {
  ...
  await Promise.all(user.userRole.permissions.map(async (permission) => permissionsValue.push(permission.dataValues.value)));

  return { role, permissionsValue };
};
...
const verifyCallback = (req, resolve, reject, requiredRights) => async (err, user, info) => {
  ...
  const { permissionsValue } = await getRolesAndPermissions(user);
